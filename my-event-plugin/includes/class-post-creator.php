...
class MEP_Post_Creator {
    ...
    public function create_event_post($form_data) {
        ...
        if (empty($selected_photo_ids) || count($selected_photo_ids) < 1) {
            return new WP_Error('invalid_photos', __('Devi selezionare almeno 1 foto', 'my-event-plugin'));
        }
        ...
        $gallery_shortcode = MEP_GDrive_Integration::create_gallery_shortcode($data['event_folder_id']);
